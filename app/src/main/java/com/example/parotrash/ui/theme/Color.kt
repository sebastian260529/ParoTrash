package com.example.parotrash.ui.theme

import androidx.compose.ui.graphics.Color

// Paleta Base
val Rojo = Color(0xFFF9342B)
val AzulNavegacion = Color(0xFF03A9F4)
val GrisDetalle = Color(0xFF666666)

// Modo Light
val FondoLight = Color(0xFFFFFFFF)
val TextoPrincipalLight = Color(0xFF000000)
val TextoBotonesLight = Color(0xFFFFFFFF)
val SuperficieLight = Color(0xFFF5F5F5) // Un gris muy tenue para tarjetas

// Modo Dark
val FondoBlack = Color(0xFF000000)
val TextoPrincipalBlack = Color(0xFFFFFFFF)
val TextoBotonesBlack = Color(0xFF000000)
val SuperficieBlack = Color(0xFF121212)